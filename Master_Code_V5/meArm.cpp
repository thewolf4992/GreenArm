...
void meArm::openGripper() {
  _gripper.write(angle2pwm(_svoGripper,pi/3)); //2nd parameter changed from pi/2 to pi/3 for wider opening
  delay(300);
}

//Let go of something
void meArm::closeGripper() {
  _gripper.write(angle2pwm(_svoGripper,-5)); //2nd parameter changed from 0 to -5 for tighter grip
  delay(300);
}
